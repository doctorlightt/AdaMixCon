...
def get_dataset(dataset_name, numComponents, target_folder='./Datasets/'):
    ...
    if dataset_name == 'PaviaU':
        ...
        img = io.loadmat(folder + 'Houston2018.mat')['houston2018']
        gt = io.loadmat(folder + 'Houston2018_gt.mat')['houston2018_gt']
        ...
        img = io.loadmat(folder + 'Houston2013.mat')['houston2013']
        gt = io.loadmat(folder + 'Houston2013_gt.mat')['houston2013_gt']
        label_values = ["Unclassified", "Healthy grass", "Stressed grass", "Synthetic grass", "Trees", "Soil", 
        ...
        label_values = ["Undefined", "Alfalfa", "Corn-notill", "Corn-mintill", "Corn", "Grass-pasture", "Grass-trees",
...
class HyperX_all(torch.utils.data.Dataset):
    def __init__(self, data, gt, dataset_name, patch_size=5, flip_argument=True, rotated_argument=True):
        super(HyperX_all, self).__init__()
        self.data = data
        self.label = gt
        self.mask = None
        self.patch_size = patch_size
    ...
    def resetGt(self, gt):
        ...
        p = self.patch_size // 2
        if self.patch_size > 1:
            self.mask = np.pad(gt, p, mode='constant')
            
        self.indices = []
        for c in np.unique(self.mask):
            if c == 0:
                continue
            c_indices = np.nonzero(self.mask == c)
            X = list(zip(*c_indices))
            self.indices += X
...
def get_sample(dataset_name, sample_size, run, ITER_NUM):
    sample_file = RESULT_DIR + dataset_name +'/' + str(sample_size) + '_' + str(ITER_NUM) + '/sample' + str(sample_size) + '_run' + str(run) + '.mat'
...
def save_sample(train_gt, test_gt, dataset_name, sample_size, run, SAMPLE_SIZE, ITER_NUM):
    sample_dir = RESULT_DIR + dataset_name + '/' + str(SAMPLE_SIZE) + '_' + str(ITER_NUM) + '/'
...
def get_gt(dataset_name, target_folder='./Datasets/'):
    ...
    if dataset_name == 'PaviaU':
        ...
        gt_PATH = folder + 'Houston2013_gt.mat'
        gt = sio.loadmat(gt_PATH)
        gt = gt['houston2013_gt']
    elif dataset_name == 'IP':
        gt_PATH = folder + 'Indian_pines_gt.mat'
        gt = sio.loadmat(gt_PATH)
...
def drawresult(y_pred,imageid,SAMPLE_SIZE,ITER_NUM,drawlabel,dir,run):
    ...
    if imageid == 'PaviaU':
        ...
        palette = np.array([[176,238,0],
                    ...
                    [139,0,0]])
        palette = palette * 1.0 / 255
    elif imageid == 'Houston2013':
        row = 349
        col = 1905
        palette = np.array([[0, 168, 132], [76, 0, 115], [0, 100, 0], [190, 255, 232], [255, 0, 0], [115, 0, 0],
                         [205, 205, 102], [137, 90, 68], [215, 158, 158], [255, 115, 223], [0, 0, 255],
                         [156, 156, 156], [115, 223, 255], [0, 255, 0], [255, 255, 0]])
        palette = palette * 1.0 / 255
    elif imageid == 'IP':
        row = 145
        col = 145
        palette = np.array([[0, 168, 132], [76, 0, 115], [0, 100, 0], [190, 255, 232], [255, 0, 0],
    ...
    X_result[np.where(drawlabel == 0), 0] = 0.0
    X_result[np.where(drawlabel == 0), 1] = 0.0
    X_result[np.where(drawlabel == 0), 2] = 0.0
